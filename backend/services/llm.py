...
def setup_api_keys() -> None:
    ...
    for provider in providers:
        # Determine the correct attribute name in the config object
        config_attribute_name = f'{provider}_API_KEY'
        if provider == 'GEMINI':
            config_attribute_name = 'GEMINI_API_KEY'  # Specific name used in config.py

        key = getattr(config, config_attribute_name, None)

        if key:
            # Set the environment variable that LiteLLM expects for each provider
            if provider == 'GEMINI':
                os.environ['GOOGLE_API_KEY'] = key
                logger.debug(f"API key for {provider} set as GOOGLE_API_KEY in environment for LiteLLM.")
            elif provider == 'OPENAI':
                os.environ['OPENAI_API_KEY'] = key
                logger.debug(f"API key for {provider} set as OPENAI_API_KEY in environment for LiteLLM.")
            elif provider == 'ANTHROPIC':
                os.environ['ANTHROPIC_API_KEY'] = key
                logger.debug(f"API key for {provider} set as ANTHROPIC_API_KEY in environment for LiteLLM.")
            elif provider == 'GROQ':
                os.environ['GROQ_API_KEY'] = key
                logger.debug(f"API key for {provider} set as GROQ_API_KEY in environment for LiteLLM.")
            elif provider == 'OPENROUTER':
                os.environ['OPENROUTER_API_KEY'] = key
                logger.debug(f"API key for {provider} set as OPENROUTER_API_KEY in environment for LiteLLM.")
            # For any new providers added to the list, ensure LiteLLM's expected env var is set here.
        else:
            logger.warning(f"No API key found in config for provider: {provider} (looked for config.{config_attribute_name})")
