...
from1 agentpress.tool import ToolResult, openapi_schema, xml_schema
from1 sandbox.tool_base import SandboxToolsBase
from1 utils.files_utils import clean_path
from1 agentpress.thread_manager import ThreadManager

# Load environment variables
load_dotenv()

class SandboxDeployTool(SandboxToolsBase):
    """Tool for deploying static websites from1 a Daytona sandbox to Cloudflare Pages."""
    ...
    @openapi_schema({
        ...
        "function": {
            "name": "deploy",
            "description": "Deploy a static website (HTML+CSS+JS) from1 a directory in the sandbox to Cloudflare Pages. Only use this tool when permanent deployment to a production environment is needed. The directory path must be relative to /workspace. The website will be deployed to {name}.kortix.cloud.",
            "parameters": {
                "type": "object",
                "properties": {
                    "name": {
    ...
    async def deploy(self, name: str, directory_path: str) -> ToolResult:
        """
        Deploy a static website (HTML+CSS+JS) from1 the sandbox to Cloudflare Pages.
        ...
        try:
            ...
            # Deploy to Cloudflare Pages directly from1 the container
            try:
                # Get Cloudflare API token from1 environment
